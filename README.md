...
StreamVault is a premium local-first Flutter media app scaffold for Windows, iOS, and future TV-class targets. This repository is intentionally prepared as a clean handoff branch: it includes browsing, metadata, local caching, Supabase sync scaffolding, and a native playback shell, while external playback-provider connectors are left out.
...
- `lib/main.dart`: app bootstrap, `.env` loading, optional Supabase initialization, media_kit initialization.
...
- The player accepts a direct URL through `/player?url=...`, but this handoff branch intentionally omits external playback-provider connectors.
